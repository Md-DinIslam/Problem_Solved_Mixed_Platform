...
int dp[1000005];
// int dp[105][1000005];
// long long Coins(int indx, int tk, vi &v){
ll Coins(int tk, vector<int> &v){
    if(tk == 0) return 1;
    // if(indx < 0) return 0;
    ...
    for(int x : v){
        // ans %= mod;
        if(tk-x >= 0)
            // ans = (ans + Coins(indx-1, tk-v[indx], v) + 1LL) % mod;
            // ans = ((ans % mod) + (Coins(tk-x, v)) % mod) % mod;
            ans = (ans + Coins(tk-x, v)) % mod;
            ans %= mod;
    }
    return dp[tk] = ans;
}
